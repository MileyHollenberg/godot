...
if [[ "$OSTYPE" == "darwin"* ]]; then
  source "/Users/mileyhollenberg/Documents/emsdk/emsdk_env.sh"
...
# fi
#
# # Source the emsdk environment script
# source /e/emsdk/emsdk_env.sh
#
...
#   if [[ "$OSTYPE" == "msys"* ]]; then
#     # Windows
#     echo "Refusing to build iOS binaries on Windows"
#     return
#   fi
#
...
#
# # Platform-specific input function
# if [[ "$OSTYPE" == "darwin"* ]]; then
#   # macOS input function
#   get_input() {
#     # Use a simple read for macOS, but read 3 characters to catch arrow keys
#     read -n 3 -t 0.1 input
#
#     if [[ "$input" == $'\E[A' ]]; then
#       choice="up"
#     elif [[ "$input" == $'\E[B' ]]; then
...
#       choice="left"
#     else
#       choice="$input"  # Store the input for other characters
#     fi
#   }
# elif [[ "$OSTYPE" == "msys"* ]]; then
#   # Windows (Git Bash) input function
#   get_input() {
#     # Use stty for Windows (Git Bash)
#     old_tty_settings=$(stty -g)
#     stty raw -echo -icanon time 0 min 0
#     read -r -n 3 -t 0.5 input
...
#   # Unsupported OS
#   get_input() {
#     echo "Unsupported OS, input may not work correctly."
#     read -n 1 choice
#   }
# fi
#
...
#   get_input # Get input based on the OS
#
#   case "$choice" in
#     "up") # Up arrow
...
#       show_menu
#       ;;
#     "right"|"left")  # Right/Left arrow, Space, Enter
